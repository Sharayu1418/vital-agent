...
import { horizonDipDeg, roundCoord, solarAltitude, sunTimesUTC } from "../app/lib/theme.js";
...
// ---------- solar altitude: what the sky colour is actually built on ----------

test("solar altitude matches the ephemeris", () => {
  // Golden values from pyephem (pressure 0, so no refraction model of its
  // own). Measured across 30 samples: mean 0.09 deg, worst 0.18 deg. Twilight
  // bands are 6 degrees wide, so that is roughly two orders of magnitude
  // finer than the thing it drives.
  // Generated by pyephem, not typed from memory. The first version of this
  // test carried numbers I had estimated rather than measured, and failed at
  // 20.28 vs an invented 25.60 — the implementation was right and the
  // expectation was fiction. A golden value that was not produced by a
  // reference is just a second opinion with more confidence.
  const CASES = [
    ["2026-08-16T12:00:00Z", 42.65, -73.76, 20.28],
    ["2026-08-16T00:00:00Z", 42.65, -73.76, -1.43],
    ["2026-08-16T12:00:00Z", 51.51, -0.13, 52.11],
    ["2026-12-21T09:00:00Z", 51.51, -0.13, 5.45],
    ["2026-08-16T06:00:00Z", -33.87, 151.21, 15.64],
    ["2026-08-16T12:00:00Z", 1.35, 103.82, -12.05],
  ];
  for (const [iso, lat, lng, expected] of CASES) {
    const got = solarAltitude(Date.parse(iso), lat, lng);
    assert.ok(Math.abs(got - expected) < 0.5,
      `${iso} at ${lat},${lng}: ${got.toFixed(2)} vs ${expected} degrees`);
  }
});

test("altitude and the sunrise time agree with each other", () => {
  // Two independent routes through the same physics. sunTimesUTC solves for
  // when the sun crosses the horizon; solarAltitude evaluates where it is at
  // a moment. If they disagree the theme and the phase would contradict — a
  // sky painted for dusk while the code believes it is day.
  for (const [lat, lng] of [[42.65, -73.76], [51.51, -0.13], [-33.87, 151.21]]) {
    const noon = Date.parse("2026-08-16T12:00:00Z");
    const solarDate = new Date(noon + (lng / 360) * 86400000);
    const { sunriseMs, sunsetMs } = sunTimesUTC(solarDate, lat, lng);
    // At the computed sunrise the sun should be at the horizon, -0.833 deg.
    for (const moment of [sunriseMs, sunsetMs]) {
      const alt = solarAltitude(moment, lat, lng);
      assert.ok(Math.abs(alt + 0.833) < 0.6,
        `at the computed crossing the sun is ${alt.toFixed(2)} deg, not -0.83`);
    }
  }
});
